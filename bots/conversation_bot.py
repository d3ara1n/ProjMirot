...
async def gm(group: Group, message: GroupMessage, sender: Member):
    ...
    if manager.getCurrentFlow(toGid(group.id)):
        await manager.getCurrentFlow(toGid(group.id)).handle(msg, sender.id, sender.memberName)
    else:
        ...
        if res[0]:
            ...
            manager.beginWith(flow(say, lambda: manager.terminal(toGid(group.id))), toGid(group.id), sender.id, sender.memberName)
